...
class KaiChatTest extends TestCase
{
    use RefreshDatabase;

    public function test_unauthenticated_kai_chat_request_is_blocked(): void
    {
        $this
    ...
    public function test_student_receives_safe_deterministic_reply(): void
    {
        ...
        $this
            ...
            ->postJson('/api/v1/kai/chat', ['message' => 'What fees are due?'])
            ->assertOk()
            ->assertJsonPath('data.reply', 'You have 1 unpaid or due fee item(s) totaling 100000.00.')
            ->assertJsonPath('data.request_id', 'req-kai-test')
            ->assertJsonPath('data.suggestions.0', 'Show my timetable')
            ->assertJsonPath('data.context_used.keys.0', 'user')
            ->assertJsonMissingPath('data.context_used.student_profile');
    }

    public function test_kai_chat_does_not_leak_another_students_data(): void
    {
        $this->travelTo('2026-07-02 08:00:00');
        ...
        $response = $this
            ...
            ->assertOk()
            ->assertJsonPath('data.reply', 'I found 1 upcoming timetable item(s) in your student context.');
